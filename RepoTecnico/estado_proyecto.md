...
3. **✅ Migración trueque abierto F1–F4 implementada** (commits locales `cd16666`, `6750c84`, `b672df7`, `e14157b` en `escrow-dsh-GCP`). Siguiente por decisión del director: aplicar la migración BD en Cloud SQL (`backend/db/migracion_trueke_abierto.sql`), minteo automático on-chain al publicar y módulo de geolocalización (mapa + favoritos de puntos) — pendientes documentados en `RepoTecnico/logica_trueke.md`.
