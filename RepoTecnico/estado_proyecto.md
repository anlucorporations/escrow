...
- **A7 — Verificación final**: Foundry (con TrueKeateNFT v2 `0x6C2d…7892` en anvil GCP con `usar()`), backend 44/44, E2E 47 passed / 3 skipped (PC-only). Pendiente por orden del director: push a los 3 repos y despliegue en GCP. Ya en GCP: BD migrada (migracion_trueke_abierto) y TrueKeateNFT v2 con `usar()` (`0x6C2d…7892`). **Pendientes de desplegar: API y web del ciclo 10** (verificado en vivo: `/truekes/:id/encuentro/aceptar`, `/…/rechazar` y `/truekes/:id/contacto` aún devuelven 404 en la API de producción; la web en producción tampoco tiene el widget de mapa ni las firmas por acción).
