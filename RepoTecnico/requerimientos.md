...
## Decisiones del director 2026-09-08/09 — trazabilidad (post-entrega)

> Actualización de documentación 2026-09-09. Cada decisión mapea a los RF/CU/D
> existentes; cuando no hay requisito previo se marca como **decisión de producto
> nueva (DP)** que queda registrada aquí y en `estado_proyecto.md`.

| Decisión del director | Qué implementa | Trazabilidad |
|---|---|---|
| **Regla del punto de encuentro**: propone el de MAYOR nivel D12 y reputación; la contraparte (menor) solo aprueba/rechaza; desempate = quien publicó (A) | `backend/api/routes/truekes.js` (`quienProponeEncuentro` + `GET /truekes/:id/encuentro/rol`); UI de rol en `/suite/intercambio` | RF-05 (encuentro del trueke) · CU-16 · DP-2026-09 (regla de propuesta) |
| **Disputas v2**: (1) la disputa nace SOLO del cierre ✗ No Conforme con formulario (motivo + fotos de evidencia) → REPORTADA; (2) contraparte conforme carga justificativo con fotos (plazo 3 días; si no → falla a favor del reclamante); (3) ambas evidencias → EN_VOTACION y se notifica a todos los socios (padrón on-chain) que votan viendo las pruebas; (4) socio que es parte del trueke NO vota; (5) veredicto por mayoría simple ANULAR (devolución total, trueke ANULADO) o VALIDO (COMPLETADO en cruz); sin votos en 5 días → ANULA; (6) notificación a los involucrados | `api/lib/flujo-disputas.js`, router `/disputas` (justificativo/no-conforme/votar/votaciones/detalle/evidencia), tablas `evidencias_disputa` y `votos_disputa`, `/suite/disputas` con flotante de votación, `/notificaciones` + campana | RF-06.1 (no conforme → disputa) · RF-14.8 (Socio resuelve) · D13/D21/D26 (quórum, votación, anulación por defecto) · DP-2026-09 (formulario con fotos, justificativo del conforme, veredicto binario ANULAR/VALIDO, exclusión de socio parte) |
| **Sistemas solo Owner**: `/suite/admin` (Sistemas) y TODAS las rutas `/admin/*` son exclusivas del Owner REAL (dueño on-chain del SociosRegistry); el icono no aparece a Socios/otros | `api/lib/es-owner.js` + middleware `requiereOwner`; `esOwner` en `/auth/estado` y `/auth/session`; navegación `visible: esOwner` | RF-13.1 (panel del Owner) · DP-2026-09 (definición de Owner = dueño on-chain, no tipo SOCIO) |
| **Botón de escalera D28 en Perfil y menú de usuario**: INSCRITO → Iniciar verificación; VERIFICADO → Iniciar certificación (KYC); CERTIFICADO → aviso | Botones en `app/suite/perfil/page.tsx` y `components/TopBar.tsx` | RF-01.5/D28 (escalera) · RF-18.4 (KYC) |
| **Intercambio por pestañas**: Activos (en curso) / Histórico (COMPLETADO/ANULADO/BLOQUEADO con etiqueta de resultado) | `app/suite/intercambio/page.tsx` | RF-05 (mis truekes) · DP-2026-09 (UX histórico) |
| **Inventario**: alta de nuevo elemento en un FLOTANTE con imágenes referenciales (1–5) | `app/suite/inventario/page.tsx` | RF-04 (publicaciones con imágenes) · DP-2026-09 (modal) |
| **Finanzas → VALOR** (3 subsecciones): 4.1 criptos del socio (Recargar/Retirar/Convertir ETH⇄BRLT contra la plataforma; sin P2P: entre socios solo vía Trueke); 4.2 reputación + trueques sin valorar (valoración inline) + últimos 10 valorados; 4.3 BRLT (Recargar con Stripe Checkout alojado / Retirar / Convertir) solo Empresa/SOCIO/Owner | Router `/valor` + tablas `movimientos_valor`/`movimientos_brlt`; valoraciones persistidas en tabla `valoraciones`; página `/suite/valor`; Stripe Checkout + webhook (claves test en Secret Manager) | RF-14.7/14.8 (saldos por rol) · D5/D6/D7 (BRLT/fondo) · D18/D36 (valoraciones) · DP-2026-09 (VALOR, contraparte plataforma, Stripe alojado, restricción Empresa/SOCIO/Owner) |
