...
/**
 * Menú Desplegable de Usuario (Mobile & Desktop).
 * 
  ...
  if (!isConnected) {
    return (
      <button
        ...
        className="btn-truekeat-primary px-5 py-2.5 text-xs uppercase tracking-wider"
      >
    ...
    <div className="relative flex items-center gap-2" ref={ref}>
      ...
      {account && (
        <div className="relative">
          ...
          {notifOpen && (
            <div className="absolute right-0 mt-3 w-80 rounded-3xl bg-white border border-slate-200 shadow-2xl shadow-navy-900/10 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
              <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <span className="font-heading font-bold text-sm text-[#1A2B4C]">Notificaciones</span>
                <span className="text-[10px] text-[#2A9D8F] font-mono font-bold uppercase">En tiempo real</span>
              </div>
              <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    <div key={n.id} className="px-6 py-3.5 text-xs hover:bg-[#F8FFFE] transition-colors">
      ...
      {/* TARJETA DESPLEGABLE DE USUARIO (Vault Dropdown) */}
      {open && (
        <div className="absolute right-0 top-full mt-3 w-84 sm:w-96 rounded-3xl bg-white border border-slate-200 shadow-2xl shadow-[#1A2B4C]/15 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150 p-2">
          
          {/* CABECERA: Identidad & Nivel */}
          <div className="p-5 bg-gradient-to-br from-slate-50 to-[#F8FFFE] rounded-2xl border border-slate-100 mb-2">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 block mb-0.5">
                  Billetera Web3
                </span>
                <h3 className="text-lg font-heading font-extrabold text-[#1A2B4C] leading-snug">
            ...
            <div className="flex items-center justify-between bg-white rounded-xl px-3 py-2 border border-slate-200/70 shadow-xs">
              <span className="font-mono text-[11px] text-slate-600 truncate max-w-[200px] sm:max-w-[230px]">
                {account}
              </span>
              <button
                onClick={copyAddress}
                className="text-[11px] font-semibold text-[#2A9D8F] hover:text-[#21867A] flex items-center gap-1 transition-colors ml-2 flex-shrink-0"
              >
                {copied ? (
            ...
            {!isRegistered && !regLoading && (
              <Link
                ...
                className="w-full mt-3 py-2.5 bg-[#D4AF37] hover:bg-[#C5A065] text-[#1A2B4C] font-bold rounded-xl text-xs tracking-wide uppercase transition-all shadow-xs block text-center"
              >
          ...
          <div className="px-2 py-1 mb-2">
            <Link
              href="/balances"
              onClick={() => setOpen(false)}
              className="flex items-center justify-between p-3.5 bg-slate-50 hover:bg-[#F8FFFE] rounded-2xl border border-slate-100 transition-all group"
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-[#1A2B4C] text-[#48CAE4] flex items-center justify-center shadow-xs">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                  </svg>
                </div>
                <div>
                  <h4 className="text-xs font-bold text-[#1A2B4C] group-hover:text-[#2A9D8F] transition-colors font-heading">
              ...
              <span className="text-xs font-semibold text-[#2A9D8F] bg-white px-2.5 py-1 rounded-full border border-slate-200 shadow-xs">
                Ver saldos →
              </span>
            </Link>
          </div>
          ...
          <div className="space-y-0.5 px-2 py-1 border-t border-slate-100">
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-slate-400 group-hover:text-[#2A9D8F] transition-colors">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-800 group-hover:text-[#2A9D8F] transition-colors font-heading">
              ...
              <>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-teal-900 group-hover:text-[#2A9D8F]">
                        Votación de Nuevos Socios
                      </p>
                      <p className="text-[10px] text-teal-600/80 font-light">
            ...
            {isAdmin && (
              <Link
                href="/add-token"
                onClick={() => setOpen(false)}
                className="flex items-center justify-between px-3.5 py-2.5 rounded-xl hover:bg-slate-100 transition-colors group"
              >
                <div className="flex items-center gap-3">
                  <span className="text-[#2A9D8F]">⚙️</span>
                  <div>
